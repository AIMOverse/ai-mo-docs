...
The X402 payment endpoint enables direct cryptocurrency payments for AI model inference without requiring pre-funded escrow accounts or API keys. Simply make requests and pay on-demand using blockchain transactions.

## Overview

This approach is ideal for:
- **Pay-per-use scenarios** without account setup
- **Blockchain-native applications** that prefer direct payments
- **Decentralized integrations** requiring permissionless access
- **Testing and experimentation** without pre-funding

The easiest way to use X402 payments is with the official X402 SDK, which handles all payment flows automatically.

## Endpoint Details

**Path:** `/alpha/chat/completions`

**Base URL:** `https://devnet.aimo.network/api`

**Full Endpoint:** `POST https://devnet.aimo.network/api/alpha/chat/completions`
...
const response = await fetchWithPay("https://devnet.aimo.network/api/alpha/chat/completions", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
  },
  body: JSON.stringify({
    model: "Df6Uz871M62ps3K9oEQuB6ZbJcuLV2AXmrLZ8uoFDutv:deepseek-r1",
    messages: [
...
The X402 endpoint accepts the same request format as the standard chat completions endpoint:
...
- `model` (required): Format is `provider_pubkey:model_name`
...
The endpoint automatically selects the payment network based on your request or returns options for both networks. You can specify a preferred network:

**Via Header:**
```bash
curl -X POST "https://devnet.aimo.network/api/v1/alpha/chat/completions" \
...
```bash
curl -X POST "https://devnet.aimo.network/api/alpha/chat/completions" \
  -H "Content-Type: application/json" \
...
| Feature | X402 Endpoint | Standard Endpoint |
|---------|--------------|-------------------|
| Authentication | None required | API key required |
| Account Setup | Not needed | Requires account |
| Payment Method | Direct blockchain tx | Pre-funded escrow |
| Payment Timing | Pay-per-request | Pre-funded balance |
| Transaction Fees | Gas fees per request | Minimal (escrow) |
| SDK Support | x402-fetch | OpenAI SDK compatible |
| Best For | One-off requests, testing, decentralized apps | High-volume usage, production |
